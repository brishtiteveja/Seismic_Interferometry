...
properties (Hidden = false)
    SIZE    = 240.0; %Size of trace header in bytes
...
properties
    ...
    NumExtTxtHeaders   = 0; % Number of Extended Textual File headers [rev 1+], default = 0
    ...
    NumExtTrcHeaders   = 0; % Number of Extended Trace Headers [rev 2], default = 0
    ...
    function obj = SegyTrace(filename,permission,fmtcode,byteorder,segyrevision,gui)
        ...
        if obj.FileID > 0 && obj.fsize > 3600
            ...
            if obj.SegyRevision >0
                ...
                obj.NumExtTxtHeaders  = bh.readNumExtHeaders();
            end
...
%                 obj.IntegerConstant    = bh.readIntegerConstant();
                obj.NumExtTrcHeaders   = bh.readNumExtTrcHeaders();
...
%                     +3200*double(bh.readNumExtTxtHeaders());
    ...
    function set.SegyRevision(obj, v)
        if isempty(v) || (isscalar(v) && isnumeric(v) && v>-1 && v<3)
            ...
            mm_errordlg('@SegyFile: SegyRevision must be numeric, scalar and = 0, 1, or 2',...
            'Error!',obj.GUI)                
        end
    end
    ...
    function fn = get.TraceSize(obj)
        if isempty(obj.SamplesPerTrace) || isempty(obj.BytesPerSample)
            ...
            fn = double(obj.SamplesPerTrace)*double(obj.BytesPerSample)+obj.SIZE;
        end
    end
    ...
    function st = get.HdrEndBytes(obj)
        ...
        if isempty(obj.HdrDef)
            ...
            st = cell2struct([obj.HdrDef(2:end,3); {obj.SIZE}],obj.HdrDef(:,1));
        end
    end %end get.HdrStartBytes
...
methods (Hidden)
    ...
    function trcdat = readData(obj,trcrange)
        ...
        if(debug), disp('starting data fread'), end
        obj.fseek(startbyte+obj.SIZE,'bof');
        trcdat = obj.fread(...
                nbytes, ...
                sprintf('%d*%s',spt,obj.FormatCodeType), ...
                obj.FormatCodeType, ...
                obj.SIZE +(skiptraces-1)*obj.TraceSize ...
            );
        if(debug), disp('finished data fread'), end
        ...
        if(debug), disp('starting data conversion'), end
        switch(obj.FormatCode)
            case(1) %IBM float
                utrcdat = trcdat;
        ...
        if(debug), disp('finishing data conversion'), end
    end
    ...
    function trcheadstruct = readHeader(obj,trcrange)
        ...
        if debug, disp('starting header fread'), end
        trchead = obj.fread(...
            [obj.SIZE, numtraces], ...
            sprintf('%d*uint8',obj.SIZE), ...
            'uint8', ...
            obj.TraceSize-obj.SIZE +(skiptraces-1)*obj.TraceSize...
            );
        if debug, disp('finished header fread'), end
    ...
    function th = newHeader(obj,ntrace,nsamp,sampint)
        ...
        if isempty(obj.HdrDef)
            obj.HdrDef = obj.newDefinition(obj.SegyRevision); %(segyrev 1 if empty);
        end
