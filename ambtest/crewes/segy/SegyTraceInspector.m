function SegyTraceInspector(segyfile,hbutton)
% GUI to inspect single traces in a SEGY file and determine endianness and data format
%
% SegyTraceInspector(segyfile,hbutton)
%
% segyfile ... full fname of file including complete relative path
% hbutton ... handle of a uicontrol into which the most recent choice of endianness and dataformat
%       will be stored in userdata.
% *********** default = [] ************* (none)
% NOTE, if hbutton is provided then the command udat=get(hbutton,'userdata') will retrun a cell
% array of length 2. udat{1} will be a string either 'b' or 'l' indicating big or
...
dt=single(sf.SampleInterval)/1000000;
t=dt*(0:length(st)-1)';
[SF,f]=fftrl(st,t);
...
uicontrol(hfig,'style','text','String','Maximum time','units','normalized','position',...
    [xnow,ynow+.6*ht,wid,ht]);
uicontrol(hfig,'style','edit','string','all','units','normalized','position',...
