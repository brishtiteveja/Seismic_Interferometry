...
if isequal(nargin,7) && ~isempty(bindef)
    obj.BinaryHeader.HdrDef = bindef;
end

if isequal(nargin,8) && ~isempty(trcdef)
...
if isempty(binhdr)
    binhdr = obj.BinaryHeader.new(obj.SegyRevision); %rev1 by default
end

[nsamp,ntrace] = size(trcdat);
binhdr.(obj.BinaryHeader.byte2word(20)) = nsamp;
binhdr.(obj.BinaryHeader.byte2word(22)) = nsamp;
sampint = binhdr.(obj.BinaryHeader.byte2word(16));

if isempty(trchdr)
    trchdr = obj.Trace.new(ntrace,nsamp,sampint,obj.SegyRevision); %rev1 by default
end
