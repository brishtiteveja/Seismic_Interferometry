...
%     end
    ...
    imagesc(x,y,seis1,clim);colormap(seisclrs)
    brighten(.5);
    grid
...
%     lw=.5;
%     xs=[x1(1) x1(end)];
%     h1=line(xs,ttop,'color','r','linestyle','-','buttondownfcn','seisplot_specdtslice(''dragline'');','tag','ttop','linewidth',lw);
%     h2=line(xs,tbot,'color','r','linestyle','--','buttondownfcn','seisplot_specdtslice(''dragline'');','tag','tbot','linewidth',lw);
 ...
 %   legend([h1 h2],'design gate top','design gate bottom','location','southeast')
    ...
    uicontrol(gcf,'style','radiobutton','string','Auto adjust clip','tag','autoclip1','units','normalized',...
        'position',[xnow,ynow,wid,ht],'value',0,'userdata',ilive,...
    ...
    uicontrol(gcf,'style','pushbutton','string','Info','tag','info','units','normalized',...
        'position',[xnow,ynow+2.5*ht,.5*wid,.5*ht],'callback','seisplot_specdtslice(''info'');',...
        'backgroundcolor','y');
    ...
    uicontrol(gcf,'style','text','string','SpecD parameters:','units','normalized',...
        'position',[xnow,ynow,wid,ht],'tooltipstring','Change these values and then click "Compute SpecD"');
    ynow=ynow-ht-sep;
    wid=wid*.5;
...
%     end
    set(gcf,'name',['Spectral decomp for ' dname ' at time= ' time2str(t(inot))],...
        'closerequestfcn','seisplot_specdtslice(''close'');');
    if(nargout>0)
        datar=cell(1,2);
        datar{1}=hax1;
        datar{2}=hax2;
...
elseif(strcmp(action,'info'))
    ...
    hinfo=msgbox(msg,'Instructions for "Spectral Decomp on time slices" tool');
