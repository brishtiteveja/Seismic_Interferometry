...
%     brighten(.5);
    ...
    uicontrol(gcf,'style','pushbutton','string','Reset windows to globals','units','normalized',...
        'position',[xnow,ynow+ht,1.5*wid,.5*ht],'callback','seisplottvs(''resetwindows'')','tag','resetwin',...
    ...
    tpad=2*max(twins);
    tvdbspec(t,seis,t1s,twins,tpad,'',hax2);  
    yl=get(gca,'ylim');
    dblimmin=yl(1);
    if(~isempty(DBLIM))
        dblim=DBLIM;
...
elseif(strcmp(action,'info'))
    ...
    msg=['The spectral windows are indicated by the colored horizontal lines on the seismic. ',...
        ...
        'relative amplitude to one another.'];
