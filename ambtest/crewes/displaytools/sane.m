...
% compared, QC'd, and perhaps en'infohanced. To make effective use of SANE you should run this on a
...
% to be a mechanism to share parmsets. Sane tasks will execute via a callback to an action in sane.
...
%     end
    ...
    if(isdeployed)
        ...
        set(hsane,'menubar','none','toolbar','none','numbertitle','off','name','SANE (MATLAB) New Project',...
            'nextplot','new','closerequestfcn','sane(''close'')');
    end
    ...
    if(~isdeployed)
        uimenu(hwrite,'label','Multiple sgy files','callback','sane(''writemanysegy'');','tag','writemanysegy');
    end
    ...
    if(isdeployed)
        uimenu(hcompute,'label','Gabor decon','callback','sane(''starttask'');','tag','gabordecon','enable','on');
    else
        uimenu(hcompute,'label','Gabor decon','callback','sane(''starttask'');','tag','gabordecon','enable','on');
    end
    ...
    uimenu(hsd,'label','Compute Spec Decomp','callback','sane(''starttask'');','tag','specdecomp','enable','on');
    if(isdeployed)
        uimenu(hcompute,'label','SVD separation','callback','sane(''starttask'');','tag','svdsep','enable','on');
    else
        uimenu(hcompute,'label','SVD separation','callback','sane(''starttask'');','tag','svdsep','enable','on');
    end
    ...
    uicontrol(gcf,'style','pushbutton','string','Info','tag','info','units','normalized',...
        'position',[0,1-hinfo,winfo,hinfo],'callback','sane(''saneinfo'');','backgroundcolor','y');
    panelwidth=1-2*x0;
...
%     SEGYTI_FORMATCODE=[];
    ...
    set(hdial,'position',[pos(1)+.5*(pos(3)-figwid),pos(2)+.5*(pos(4)-fight),figwid,fight],...
        ...
        'closerequestfcn','sane(''readsegy2cancel'');');
    xnot=.05;ynow=.85;
    xnow=xnot;
    wid=.9;ht=.09;
    ysep=.01;xsep=.02;
    ...
    uicontrol(hdial,'style','pushbutton','string','Trace inspector','tag','inspector',...
        ...
        'callback','sane(''starttraceinspector'');')
    ...
    uicontrol(hdial,'style','pushbutton','string','Info','tag','inspector',...
        ...
        'callback','sane(''helpsegyin'');','backgroundcolor','y')
...
%             [],[],[],[],[],{hsane});
    ...
    if(strcmp(proj.name,'New Project'))
        ...
        if(isdeployed)
            ...
            set(hsane,'name',['Sane, (MATLAB) Project: ' proj.name])
        end
    end
...
elseif(strcmp(action,'datainfo'))
    ...
    uicontrol(hinfofig,'style','pushbutton','string','History','units','normalized',...
        'position',[xnow,ynow,.75*wid,ht],'callback','sane(''showhistory'');');
...
elseif(strcmp(action,'projectnamechange'))
    ...
    if(isdeployed)
        ...
        set(gcf,'name',['SANE, (MATLAB) Project: ' proj.name])
    end
    set(hfile,'userdata',proj);
elseif(strcmp(action,'saveproject')||strcmp(action,'saveproj'))
    ...
    if(isdeployed)
        ...
        set(hsane,'name',['SANE, (MATLAB) Project file: ' filename]);
    end
...
%                     proj.ycoord{ilive(idata)},proj.datanames{ilive(idata)},'seisclrs',proj.dx(ilive(idata)),proj.dy(ilive(idata)))
                ...
                set(gcf,'closeRequestFcn','sane(''closepifig'');')
                hview=findobj(hsane,'tag','view');
                uimenu(hview,'label',proj.datanames{ilive(idata)},'callback','sane(''popupfig'');','userdata',gcf);
...
elseif(strcmp(action,'starttask'))
    ...
    switch task
        ...
        case 'gabordecon'
            sanetask(proj.datanames,parmset,task);
        case 'svdsep'
            svdsep_dialog(parmset,'sane(''dotask'');');%svd_sep need a more complex dialog than the standard
    end
    hmsg=findobj(hsane,'tag','message');
...
%             HWAIT=waitbar(0,['Gabor decon beginning inline 1 of ' int2str(ny)],'CreateCancelBtn','sane(''canceltaskafter'')');
            t0=clock;
            ...
            if(strcmp(task,'specdecomp'))
                ...
                for k=1:length(amp)
                    ...
                    writesegy([path fname2],seis,getsegyrev(idat),dt,proj.segfmt{idat},proj.texthdrfmt{idat},...
                        ...
                        trch,proj.bindef{idat},proj.trcdef{idat},hsane);
...
%                     ' saved and displayed. Data will be written to disk when you save the project.'])
                ...
                set(gcf,'closeRequestFcn','sane(''closepifig'');')
                hview=findobj(hsane,'tag','view');
                uimenu(hview,'label',proj.datanames{ndatasets},'callback','sane(''popupfig'');','userdata',gcf);
                proj.pifigures{ndatasets}=gcf;
...
elseif(strcmp(action,'saneinfo'))
    hsane=gcf;
    hmsg=findobj(hsane,'tag','message');
    msg={['SANE, or Seismic ANalysis Environment, is a tool designed to facilitate the anlysis, comparison, ',...
        'and enhancement of 3D seismic volumes. To use SANE you will need to have one or more 3D seismic ',...
        'datasets available as SEGY files. SANE lets you define "Projects" which contain one or more related 3D ',...
        ...
        [],['Before loading data into SANE, it is useful to understand ',...
        ...
        'of the computer memory then they are probably too large for SANE. You can load any number of SEGY ',...
        'volumes into a SANE Project even if their combined size exceeds your computer memory. This is because ',...
        'SANE gives you control over which datasets in a Project are actually loaded into memory. '],...
        ...
        'This is easily done by clicking the button labelled "Trace headers". By default, SANE will ',...
        ...
        [],['Each dataset in a project will have a unique row, or subpanel, in the main data panel of the SANE window, stating its name, ',...
        ...
        [],['SANE has the ability to automatically make PowerPoint slides of almost any view that you see. ',...
        'If you push the "Start PPT" button on the main SANE window this initiates the process. ',...
        ...
        [],['When you choose to display a particular dataset within SANE it appears in a separate window ',...
        ...
        'accessed through the "Compute" menu in the main SANE window.']};
     hinfo=msgbox(msg,'Information for SANE');
...
elseif(strcmp(get(hfig,'tag'),'fromsane'))
    ...
    %sane figure handle. (The first entry is lost in time.) With some plotimage3D tools, this tag is
...
uicontrol(hdial,'style','pushbutton','string','Info','units','normalized','position',...
    [.9,ynow+ht,.3*wid2,ht],'backgroundcolor','y','callback','sane(''helpsegyin'');');
...
%scrollbar
uicontrol(hdial,'style','slider','tag','slider','units','normalized','position',...
    [xnow+1.01*panelwidth,ynow,.025,panelht],'value',1,'Callback',{@sane_slider,hdp});
...
set(hdial,'position',[pos(1) y0 widfig htfig],'menubar','none','toolbar','none','numbertitle','off',...
    'name','SANE: Multiple SEGY write dialogue','closerequestfcn','sane(''cancelmultiplewrite'');');
xnow=.1;ynow=.85;
wid1=.8;ht1=.05;
...
set(hdial,'userdata',{hsane,checkboxes})
ynow=.22;
wid2=.1;ht2=.05;
uicontrol(hdial,'style','pushbutton','string','Define output folder','units','normalized',...
    ...
    'callback','sane(''definewritefolder'');','foregroundcolor','r','fontweight','bold');
ynow=ynow-ht2;
...
uicontrol(hpan,'style','pushbutton','string','Trace inspector','units','normalized','position',...
    [xn,yn,1.25*wid2,ht],'callback','sane(''starttraceinspector'');',...
...
WinOnTop(htask,true);
set(htask,'name',['SANE: ' taskname ' dialog'],'closerequestfcn','sane(''canceltask'')');
...
set(htask,'userdata',{[],hsane});%first space is in case the dialog evovles to launch secondary figures
...
if(strcmp(action,'init'))
    ...
    x1=ulx;
    y1=uly-figheight;
    %for compatibility with the "sanetask" dialog, the userdata of the figure 
    hdial=figure('position',[x1 y1 figwidth figheight],'menubar','none','toolbar','none','numbertitle','off',...
        'name','SVD separation: Parameter Dialog','userdata',{task,hsane},'tag','dialog',...
