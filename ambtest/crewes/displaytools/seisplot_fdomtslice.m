...
%     end
    ...
    ynow=ynow-4*ht-sep;
    hbg=uibuttongroup('position',[xnow,ynow,wid,4*ht],'title','Display choice','tag','choices',...
    ...
    uicontrol(gcf,'style','text','string','Fdom parameters:','units','normalized',...
        'position',[xnow,ynow,wid,ht],'tooltipstring','Change these values and then click "Compute Fdom"');
    ynow=ynow-ht-sep;
    wid=wid*.5;
    ...
    uicontrol(gcf,'style','pushbutton','string','Compute Fdom','units','normalized',...
        'position',[xnow,ynow,2*wid,ht],'callback','seisplot_fdomtslice(''computefdom'');',...
...
%     end
    set(gcf,'name',['Fdom for ' dname ' at time= ' time2str(t(inot))],...
        'closerequestfcn','seisplot_fdomtslice(''close'');');
    if(nargout>0)
        datar=cell(1,2);
        datar{1}=hax1;
        datar{2}=hax2;
...
%     amin=udat{5};
    ...
    if(iclip==1)
        ...
        if(isgraphics(tmp))
            ...
            'numbertitle','off','name','Colorbar limits chooser','tag','climchooser','userdata',hmasterfig);
...
elseif(strcmp(action,'info'))
    ...
    hinfo=msgbox(msg,'Instructions for "Dominant Frequency on time slices" tool');
