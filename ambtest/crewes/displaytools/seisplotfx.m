...
%     brighten(.5);
    ...
    uicontrol(gcf,'style','pushbutton','string','Info','tag','info','units','normalized',...
        'position',[xnow,ynow+2*ht,.5*wid,.5*ht],'callback','seisplotfx(''info'');',...
        'backgroundcolor','y');
    %make a clip control
    ynow=ynot+yht-ht;
...
%         'tooltipstring','separate the spectra for easier viewing','userdata',0);
    ...
    if(flag==0)
        ...
        set(gcf,'name',['F-X amplitude analysis for ' dname2],'userdata',-999.25);%userdata here is just a placeholder to cause plotimage3D to do the right thing
    else
        if(~iscell(dname))
            dname2=dname;
        else
        ...
        set(gcf,'name',['F-X phase analysis for ' dname2],'userdata',-999.25);%userdata here is just a placeholder to cause plotimage3D to do the right thing
    end
    if(nargout>0)
        datar=cell(1,2);
        datar{1}=hax1;
...
elseif(strcmp(action,'info'))
    ...
    hinfo=msgbox(msg,'Instructions for f-x spectra tool');
