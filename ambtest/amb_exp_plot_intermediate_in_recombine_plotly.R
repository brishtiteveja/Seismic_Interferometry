...
proccc <- addTransform(byamp, function(v) {
  ...
  #v = stations[[1]]
  v$amp[is.na(v$amp)] <- 0
  plot_ly(x=~(1:n), y=~v$amp, type='scatter', mode='lines') 
  
  a = v$amp -  mean(v$amp)
  plot_ly(x=~(1:n), y=~a, type='scatter', mode='lines', color='red') 
  
  
  a = detrend(a)
  a = v$amp -  mean(v$amp)
  plot_ly(x=~(1:n), y=~a, type='scatter', mode='lines', 
          line=list(color='rgb(205, 12, 24)')) 
  
  a = filtfilt(b1, a, type="pass")
  b = signbit(a)
  plot_ly(x=~(1:n), y=~b, type='scatter', mode='lines', 
          line=list(color='rgb(227, 100, 255)'))
  
  
  
  au_sta_22  = acf(b,lag.max = l-1, type = c("correlation"))
  x = as.numeric(au_sta_22$lag)
  y = as.numeric(au_sta_22$acf)
  plot_ly(x=~x, y=y,
  ...
  x = as.numeric(1:length(wh_sta_22))
  y = as.numeric(wh_sta_22)
  plot_ly(x=~x, y=y,
          type='scatter', mode='lines')
  ...
  x = as.numeric(1:length(result_station_22))
  y = as.numeric(result_station_22)
  plot_ly(x=~x, y=y,
          type='scatter', mode='lines')  
